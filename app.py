...
def make_log_plot(df, depth_col, gr_col, rhob_col, nphi_col, res_col, lith_pred):
    ...
    if res_col and res_col in df.columns:
        ...
        fig.update_xaxes(title_text="Res (ohm·m)", type="log", row=1, col=3)

    # Lithology track (colored)
    ...
    fig.update_layout(
        ...
        showlegend=False,
        height=1100,  # ⬅ الرسم الطويل كما طلبت C
...
def make_crossplot1(df, gr_col, res_col, lith_pred):
    """GR vs Resistivity – ملونة حسب lithology"""
    fig = go.Figure()
    ...
    if not res_col or res_col not in df.columns:
        return fig.update_layout(
            template="plotly_dark",
            height=360,
            title="GR vs Resistivity (no RES column)",
        )

    lith_series = pd.Series([str(l).lower() for l in lith_pred])
    unique_lith = lith_series.unique()

    for lith in unique_lith:
        mask = lith_series == lith
        fig.add_trace(
            go.Scatter(
                x=df.loc[mask, gr_col],
                y=df.loc[mask, res_col],
                mode="markers",
                name=lith.capitalize(),
                marker=dict(
                    color=COLOR_MAP.get(lith, "#ffffff"),
                    size=5,
                    opacity=0.8,
                ),
            )
    ...
    fig.update_xaxes(title_text="Gamma Ray (API)")
    fig.update_yaxes(title_text="Resistivity (ohm·m)", type="log")
    fig.update_layout(
        template="plotly_dark",
        height=360,  # ⬅ الباقيات 3 = 360px
        title="GR vs Resistivity (AI Lithology Colors)",
...
def make_crossplot2(df, rhob_col, nphi_col, lith_pred):
    """RHOB vs NPHI – ملونة حسب lithology"""
    fig = go.Figure()
    ...
    for lith in unique_lith:
        ...
        fig.add_trace(
            go.Scatter(
                x=df.loc[mask, rhob_col],
                y=df.loc[mask, nphi_col],
                mode="markers",
                name=lith.capitalize(),
                marker=dict(
                    color=COLOR_MAP.get(lith, "#ffffff"),
    ...
    fig.update_yaxes(title_text="NPHI (v/v)")
    fig.update_layout(
        template="plotly_dark",
        height=360,
        title="RHOB vs NPHI (AI Lithology Colors)",
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        margin=dict(l=60, r=20, t=40, b=40),
    )
    return fig
...
def make_cluster3d(df, gr_col, rhob_col, nphi_col):
    """3D clusters باستخدام KMeans"""

    fig = go.Figure()

    cols = [gr_col, rhob_col, nphi_col]
    df_xyz = df[cols].dropna()
    if len(df_xyz) < 20:
        fig.update_layout(
            template="plotly_dark",
            height=360,
            title="3D AI Clusters (insufficient data)",
        )
        return fig
...
def analyze_welllog():
    try:
        ...
        # ----- BUILD PLOTS -----
        fig_logs = make_log_plot(df, depth_col, gr_col, rhob_col, nphi_col, res_col, lith_pred)
        fig_cp1 = make_crossplot1(df, gr_col, res_col, lith_pred)
        fig_cp2 = make_crossplot2(df, rhob_col, nphi_col, lith_pred)
        fig_3d = make_cluster3d(df, gr_col, rhob_col, nphi_col)

        summary = (
            f"OILNOVA Well Log AI – Smart Interpretation Summary\n"
            f"Total samples: {len(df)}\n\n"
            "Lithology distribution:\n"
            + "\n".join(f"  - {k}: {v}" for k, v in lith_counts.items())
            + f"\n\nNet Pay (approx): {net_pay:.2f} depth units\n"
            f"Pay detection source: {pay_source}\n"
            f"Average AI porosity in pay zones: {avg_phi_pay:.3f}\n"
            f"Missing logs filled (ML): {fill_info}"
        )
        ...
        return jsonify(
            {
                ...
                "plots": {
                    "main_logs": json.dumps(fig_logs, cls=PlotlyJSONEncoder),
                    "crossplot1": json.dumps(fig_cp1, cls=PlotlyJSONEncoder),
                    "crossplot2": json.dumps(fig_cp2, cls=PlotlyJSONEncoder),
                    "cluster3d": json.dumps(fig_3d, cls=PlotlyJSONEncoder),
                },
                "pdf_report": {
